...
imgBackground = cv2.imread("Resources/backgroundMe.png")  # Import the Background Image
...
while True:
    ...
    # Take Encodings & Check New Faces Whether it's Match or not.
    ...
    # Feed THe Value into Face Recognition System to Detect & Give Output.
    # To do this, We Need Face & Encoding in the Current Frame.
    ...
    for encodeFace, face_location in zip(encodeCurFrame, faceCurFrame):
        ...
        if counter != 0:
            if counter == 1:
                # Access or Download Data from Database
                studentsInfo = db.reference(f"Students/{iD}").get()
                print(studentsInfo)
                # Access Images From Database Storage
            ...
            offset = (414 - w) // 2
            cv2.putText(imgBackground, str(studentsInfo['name']), (995, 498), cv2.FONT_HERSHEY_COMPLEX, 0.6,
                        (50, 50, 50), 1)
